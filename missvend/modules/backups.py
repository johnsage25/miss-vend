...
import missvend.modules.sql.notes_sql as sql
from missvend import dispatcher, LOGGER, OWNER_ID, SUDO_USERS, MESSAGE_DUMP
from missvend.__main__ import DATA_IMPORT
from missvend.modules.helper_funcs.chat_status import user_admin
from missvend.modules.helper_funcs.misc import build_keyboard, revert_buttons
from missvend.modules.helper_funcs.msg_types import get_note_type
from missvend.modules.rules import get_rules
import missvend.modules.sql.rules_sql as rulessql
from missvend.modules.sql import warns_sql as warnssql
import missvend.modules.sql.blacklist_sql as blacklistsql
from missvend.modules.sql import disable_sql as disabledsql
from missvend.modules.sql import cust_filters_sql as filtersql
import missvend.modules.sql.welcome_sql as welcsql
import missvend.modules.sql.locks_sql as locksql
from missvend.modules.connection import connected
...
def export_data(bot: Bot, update: Update, chat_data):
    ...
    f=open("missvend{}.backup".format(chat_id), "w")
    ...
    try:
        ...
        pass
    bot.sendDocument(current_chat_id, document=open('missvend{}.backup'.format(chat_id), 'rb'), caption="*Successfully imported backup:*\nChat: `{}`\nChat ID: `{}`\nOn: `{}`\n\nNote: This  is specially made for notes.".format(chat.title, chat_id, tgl), timeout=360, reply_to_message_id=msg.message_id, parse_mode=ParseMode.MARKDOWN)
    os.remove("missvend{}.backup".format(chat_id)) # Cleaning file
